...
Page({
  ...
  onLoad() {
    ...
    if (shareCompanyName) {
      that.setData({
        ...
        ['form.invitationToken']:shareInvitationToken
      })
    }
    that.setData({
      faceLabel: faceLabel,
      ...
      success: res => {
        ...
        if (res.data.code == "SUCCESS" && res.data.type == "WX_PERSON_MY_COMPANY_INFO") {
          ...
          if (!res.data.body.faceImg) {
            res.data.body.faceImg = localImgUrl;
          }
  ...
  onShow(options) {
    const that = this;
    let invitationImg = wx.getStorageSync('invitationImg');
    let faceEnabled = wx.getStorageSync('faceEnabled');
    let idNoEnabled = wx.getStorageSync('idNoEnabled');
    let idNoLabel = wx.getStorageSync('idNoLabel');
    if (idNoEnabled) {
    ...
    if ("0" === invitationImg && faceEnabled) {
      that.setData({
        uploadInfo: false
      })
      wx.setStorageSync("invitationImg", "1");
    } else {
      that.setData({
        uploadInfo: true
      })
     ...
     success: function(res) {
       ...
       if ("SUCCESS" == res.data.code && "WX_VERIFICATION_TOKEN" == res.data.type) {
         if('SUCCESS' != res.data.body){
           if('admin' == wx.getStorageSync('roleName')){
             wx.switchTab({
               url: '../home/home'
             })
           }else{
      ...
      wx.request({
        ...
        data: {
          ...
          body: {
            ...
            invitationToken:e.detail.value.invitationToken
